...
```{r}
t.test(as.data.frame(ptest_1), as.data.frame(ptest_2))
```
...
```{r}
p <- ggplot2::ggplot(stressshift::stress_shift_unamb, 
        ggplot2::aes(x=Category, fill=Syllable)) + ggplot2::geom_bar(colour="black")
...
stress_shift_byword = dplyr::summarise(data_regroup, Noun_Percent_Syll_1=  (sum(Syllable_Noun=="Syllable 1")/n())*100, 
